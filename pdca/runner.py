...
from pdca import config, llm, phases, state
...
def run(task: str, workdir: str, loop: bool, max_cycles: int,
        max_seconds: int, max_tokens: int) -> int:
    """Exit codes: 0 ADOPT / single-cycle done, 1 proven infeasible,
    2 budget stop (tokens/time/cycles)."""
    ...
    if missing:
        print(f"ENVIRONMENT ERROR: commands not found: {missing}")
        print("Make sure the venv is in PATH: PATH=$PWD/.venv/bin:$PATH pdca ...")
        return 3
    ...
    while True:
        ...
        print(f"--- cycle {cycle} | elapsed {int(time.monotonic() - start)}s | "
        ...
        print(f"[PLAN]  {len(plan.steps)} steps, {len(plan.success_criteria)} criteria, "
        ...
        print(f"[DO]    script ran {evidence['elapsed']}s, "
        ...
        if should_audit:
            feas = phases.feasibility_audit(task, state.load_digest(workdir, 4),
                                            last_check_pack)
            print(f"[AUDIT] feasibility: {feas.verdict} — {feas.reason}"
