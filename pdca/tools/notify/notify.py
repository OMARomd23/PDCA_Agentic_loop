...
Called once at the end of a run to email a status report. Notifications are
opt-in: the runner only invokes this when the user enabled them and supplied a
recipient (`--notify --notify-to <email>`). Orchestration only — it asks the
model to compose the prose (composer), renders the template, and delivers via
Resend (sender). Any failure is logged and swallowed: a notification problem
must never change a run's exit code.
"""
...
def notify_run_complete(*, recipient: str, task: str, workdir: str, trigger: str,
                        ...
                        state_digest: str = "") -> bool:
    """Compose and send the end-of-run status email to `recipient`. Returns True
